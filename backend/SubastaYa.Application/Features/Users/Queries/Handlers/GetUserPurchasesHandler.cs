...
public class GetUserPurchasesHandler : IQueryHandler<GetUserPurchasesQuery, IEnumerable<AuctionResponseDto>>
{
    private readonly IAuctionRepository _auctionRepository;

    public GetUserPurchasesHandler(IAuctionRepository auctionRepository)
    {
        _auctionRepository = auctionRepository;
    }

    public async Task<IEnumerable<AuctionResponseDto>> HandleAsync(GetUserPurchasesQuery query, CancellationToken cancellationToken = default)
    {
        var auctions = await _auctionRepository.GetParticipatedByUserIdAsync(query.UserId, cancellationToken);
